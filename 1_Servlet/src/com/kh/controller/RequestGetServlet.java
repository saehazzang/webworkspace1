...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * (사용자가 입력한 값, 요청전송방식, 요청한 사용자의 ip등등)
		 * 
		 * 두번째 매개변수인 HttpServletResponse response에는 요청처리 후 응답을 할 때 사용하는 객체
		 * 
		 * 우선, 요청을 처리하기 위해 요청시 전달된 값(사용자가 입력한 값)들을 뽑는다.
		 * request의 parameter 영역안에 존재 => key-value 세트로 담겨있다 (name속성값-value속성값)
		 * 
		 * 따라서 request의 parameter영역으로부터 전달된 데이터를 뽑는 메소드로
		 * - request.getParameter("키값") : String(그에 해당하는 value값) 
		 *   => 무조건 문자열 형태로 반환되기 떄문에 다른자료형으로 변경하려면 파싱해줘야함
		 * - request.getParameterValues("키값") : String[](그에 해당하는 value값들) 
		 *   => 하나의 key값으로 여러개의 value값들을 전달받을경우(체크박스) 문자열 배열형으로 반환가능
		 * */
		String name = request.getParameter("name"); // "민경민", ""(텍스트 상자가 빈 경우 빈 문자열이 넘어간다)
		...
		// 체크박스와 같이 복수개의 정보를 받을때는 배열로 받아야 한다.
		String[] foods = request.getParameterValues("food"); // ["한식","일식"] / null (체크안한다면 null 반환)
		
		/*
		 * 이 뽑아낸 값들을 가지고 요청처리해줘야 함(db와 상호작용을 함)
		 ...
		 * Sevice와 Dao간은 거의 변동없이 View와 Controller간의 소스코드상의 변화가 많을것이다
		 * 
		...
		//1) 이제부터 내가 출력할(응답할) 내용은 문서형태의 HTML이고/ 문자셋은 UTF-8이라는것을 지정
